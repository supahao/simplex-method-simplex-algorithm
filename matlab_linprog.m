...
if isempty(options)
    ...
    elseif isstruct(options)
        ...
        error('optim:linprog:InvalidOptions', ...
...
% Check for struct input first
if ~strcmpi(optimgetFlag,'optimoptions')
    largescaleOn = strcmpi(optimget(options,'LargeScale',defaultopt,optimgetFlag),'on');
    if ~largescaleOn
...
switch optimget(options,'Display',defaultopt,optimgetFlag)
...
elseif strcmpi(Algorithm,algDSX) || strcmpi(Algorithm,algIP15b)
    ...
    if strcmpi(Algorithm,algDSX)
        algoptions.Preprocess = optimget(options,'Preprocess',defaultopt,optimgetFlag);
        algoptions.MaxTime = optimget(options,'MaxTime',defaultopt,optimgetFlag);
        if ischar(thisMaxIter) && ...
    ...
    algoptions.Diagnostics = optimget(options,'Diagnostics',defaultopt,optimgetFlag);
    algoptions.Display = optimget(options,'Display',defaultopt,optimgetFlag);
    thisTolCon = optimget(options,'TolCon',defaultopt,optimgetFlag);
    if ~isempty(thisTolCon)
        algoptions.TolCon = thisTolCon;
    end
    thisTolFun = optimget(options,'TolFun',defaultopt,optimgetFlag);
